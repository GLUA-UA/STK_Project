 ...
 */
vvoid World::updateWorld(int ticks)
{
#ifdef DEBUG
    ...
    try
    {
        ...
        static int map_socket = -1;
        static struct sockaddr_in map_servaddr;

        if (map_socket == -1) {
            map_socket = socket(AF_INET, SOCK_DGRAM, 0);            
            int broadcastEnable = 1;
            setsockopt(map_socket, SOL_SOCKET, SO_BROADCAST, &broadcastEnable, sizeof(broadcastEnable));

            memset(&map_servaddr, 0, sizeof(map_servaddr));
            map_servaddr.sin_family = AF_INET;
            map_servaddr.sin_port = htons(9999);
            map_servaddr.sin_addr.s_addr = inet_addr("255.255.255.255");
        }

        int current_ticks = getTicksSinceStart();

        if (m_karts.size() > 0 && (current_ticks % 2 == 0)) {

            //if (current_ticks % 100 == 0) {
            //    std::cout << "[MAP-DEBUG] Enviando dados de " << m_karts.size() << " karts..." << std::endl;
            //}

            std::string track_id = Track::getCurrentTrack() ? Track::getCurrentTrack()->getIdent() : "unknown";

            for (unsigned int i = 0; i < m_karts.size(); i++) {
                AbstractKart* k = m_karts[i].get();
                ...
                int sent = sendto(map_socket, msg.c_str(), msg.size(), 0, 
                                  (struct sockaddr*)&map_servaddr, sizeof(map_servaddr));
                
                //if (sent < 0 && current_ticks % 100 == 0) {
                //    std::cout << "[MAP-DEBUG] Erro ao enviar UDP!" << std::endl;
                //}
            }
        }
    }
