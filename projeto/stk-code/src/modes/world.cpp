...
void World::updateWorld(int ticks)
{
    ...
    try
    {
        ...
        std::cout << "[MAPA] updateWorld executado\n";
        ...
        if (send_sock == -1) {
            std::cout << "[MAPA] Criando sockets UDP\n";
        ...
        char buffer[16];
        if (recvfrom(listen_sock, buffer, sizeof(buffer), 0, (struct sockaddr*)&client_addr, &addr_len) > 0) {
            target_addr = client_addr;
            target_addr.sin_port = htons(9999);
            has_client = true;
            std::cout << "[MAPA] Novo cliente registrado: " << inet_ntoa(target_addr.sin_addr) << std::endl;
        }

        int current_ticks = getTicksSinceStart();

        if (has_client && m_karts.size() > 0 && (current_ticks % 2 == 0)) {
